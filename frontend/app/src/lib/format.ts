...
/** 지표 문자열을 «수치» 와 «단위» 로 가른다 — "53.5만 건" -> ["53.5", "만 건"],
 *  "20,148" -> ["20,148", ""]. 수치를 크게, 단위를 작게 찍어 자릿수가 먼저
 *  읽히게 하려는 것이다(KBFG 는 Medium 이 최대 굵기라 크기가 유일한 위계다).
 *
 *  뒷부분에 숫자가 또 나오면 단위가 아니라 문장이므로("77% vs 28%") 통째로
 *  돌려준다. 여기서 억지로 쪼개면 "77" 만 커지고 뒤 절반이 각주처럼 눌린다. */
