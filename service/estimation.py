...
# 폭의 양 끝을 특정 시나리오(«못 건지면»/«건지면»)로 귀속하지 않는다 — 밴드는
# 회수·상각기간·이자율 세 축을 함께 흔든 27조합의 5~95백분위라(cost.py), 끝값이
# 어느 한 가정의 값과 일치하지 않는다. 대표값만 전액 손실 가정으로 못박는다.
ESTIMATE_NOTICE = (
    "매출은 이 상권에서 같은 업종이 올린 최신 분기 통계예요. "
    "이 매물이 얼마나 팔지 예측하거나 매물별로 나눈 값은 아니에요. "
    "대표 금액은 권리금을 한 푼도 못 건진다고 본 쪽이고, 폭은 권리금 회수·"
    "상각 기간 ±1년·이자율 ±1%p를 함께 흔들었을 때의 5~95% 구간이에요. "
    "승계 확률은 이어받을 가능성이지 권리금을 돌려받는 비율이 아니에요. "
    "관리비는 자료가 없어서 빼고 셌어요."
)
