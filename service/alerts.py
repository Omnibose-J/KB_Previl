...
#
#   등급 폭 중앙값          0.0285   (중간 등급들은 0.018~0.033)
#   |점수 이동| 중앙값      0.0174   p90 0.0531
#
# 즉 보통 칸이 6개월에 등급 폭의 61% 를 움직인다. 그래서 «등급 1칸 변동»은
# 20,117 중 11,688(58%)에서 일어나고, 그것을 전부 알리면 알림이 곧 소음이 된다.
# 등급은 연속 점수에 얹은 거친 라벨이라 경계 근처에서는 실제 변화가 작아도
# 라벨이 바뀐다.
#
# 그래서 두 조건을 함께 요구한다 — 등급이 2칸 이상 움직였고(1칸은 정상 드리프트
# 범위 안이다), 점수도 등급 폭 하나(0.0285) 이상 움직였을 것. 실측에서 2칸
# 이상은 3,584건이고 그들의 |점수 이동| 중앙값은 0.0488 로 드리프트와 확실히
# 구분된다.
