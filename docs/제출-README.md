...
화면 **「기록 자세히」 탭 맨 위의 «AI는 이 자리를 이렇게 요약했어요»**
(`POST /api/report`)가 켜진다. 키가 없으면 그 카드만 오류를 표시하고 나머지는
그대로다.

**숫자는 전부 서버가 계산하며 LLM 은 문장만 만든다.** 모델은 `{{grade}}` 같은
placeholder 로만 값을 인용할 수 있고 숫자 글리프를 직접 쓰면 거부된다 —
치환은 서버가 한다(`service/reporting.py`). 「누구와 오는 자리인가」 카드의 라벨은
사전 계산돼 DB 에 들어 있으므로 키 없이도 나온다.
...
1. **1등급 자리에서도 약 20%가 3년 내 폐업**한다. 9등급에서도 10.1%는
   살아남는다(n=317, 95% CI 7.2–13.9%).
2. **AUC 0.6383 은 «무작위보다 나은 수준»**이지 «잘 맞힌다»가 아니다. 이전
   원천의 0.6369에서도 학습 데이터 2.7배·정규화 24설정·하이퍼파라미터
   70조합·딥러닝을 시도했지만 홀드아웃 상위10%는 움직이지 않았다. 7/31
   원천 재적재는 같은 구조를 다시 적합해 0.6383을 얻었다
   (`docs/figures/fig7-learning-curve.png`는 이전 원천의 학습곡선 기록).
