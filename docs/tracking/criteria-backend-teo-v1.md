...
> 층 배수를 매출에 곱하지 말 것. 당시에는 대안으로 «생존율에는 실측치가 있다»
> (지하 59.8% / 1층 66.2% / 2층 69.9%)를 제시했으나 **그 세 값은 2026-08-01
> 폐기됐다**(F-A5 — 출처 테이블에 개·폐업일이 없었고, 재측정하면 면적 층화로
> 층 차이가 사라진다). 층에는 곱할 수 있는 검증된 값이 매출에도 생존율에도 없다.
