...
9등급에서 1등급이 전체의 4.5%가 되면서 자치구 25곳 중 한식 1등급이 20장 페이지를
채우는 곳은 7곳뿐이고 18곳이 미달이다(까페 21곳·일식 15곳 미달). 문턱을 2등급까지
열어 미달을 한식 7·까페 4·일식 1로 줄였지만, 2등급 실측은 72.9%로 옛 등분위
1등급(76.9%)보다 낮아 배지가 약속하는 값이 그만큼 내려갔다.

*(2026-08-01 수치 재측정. 종전 «28곳 중 7곳에 1등급이 아예 없고 21곳 미달, 1~2등급이면
빈 자치구 3곳» 은 자치구 수부터 틀렸고, 「3등급까지 열어도 그대로」도 지금은 거짓이다 —
1~3등급이면 12업종 전부 빈 자치구가 0이 된다.)*
...
**후속 (2026-08-01)**: 이 반려의 전제가 사라졌다. §J-1 재판정에서 코퍼스가
라벨러·판정 기준 양쪽으로 섞여 0.633·0.700 이 전체를 설명하지 못하게 됐다.
그래서 화면 문구를 뺐고(§J-1), 이어서 **API 도 `precision` 을 null 로 내린다** —
검정되지 않은 수치를 계약에 남겨 두면 «쟀다»가 «맞다»로 읽히는 통로가 화면에서
JSON 으로 옮겨갈 뿐이다. 상수(`PARTY_PILOT_PRECISION`)와 필드는 남아 있어,
단일 라벨러 재라벨 + 판정자 2인 검정을 다시 받으면 서버만 채우면 된다.
