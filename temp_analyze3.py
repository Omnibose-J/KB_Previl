...
def has_complaint(text, category):
    ...
    if category == "wait":
        ...
        if "대기" in text_lower:
            # "대기 없이", "대기 바로" = 칭찬 (반대)
            if any(p in text_lower for p in ["대기 없", "대기없", "대기 바로"]):
                return False
            # "대기가 길", "웨이팅 길", "웨이팅 많", "줄 길" = 불만
            if any(p in text_lower for p in ["대기가 길", "대기 길", "웨이팅 길", "웨이팅 많"]):
                return True
        if any(w in text_lower for w in ["웨이팅", "줄"]):
            if any(w in text_lower for w in ["길", "오래", "많"]):
                return True
        return False
    elif category == "seat":
        if any(p in text_lower for p in ["좁", "불편", "협소", "자리 없", "자리가 없"]):
            return True
        return False
        ...
        if "주차" in text_lower:
            if any(p in text_lower for p in ["어렵", "없", "협소"]):
                if "주차 가능" not in text_lower and "무료 주차" not in text_lower and "주차 있" not in text_lower:
                    return True
        return False
    return False
...
def is_restaurant_visit(text, name):
    ...
    visit_keywords = ["메뉴", "음식", "접시", "먹", "맛", "음료"]
