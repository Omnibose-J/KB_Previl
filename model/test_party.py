...
def test_fetch_failure_raises_instead_of_returning_empty():
    """빈 리스트로 돌려주면 «검색 결과 없음» 마커가 쓰이고, 그 상권은 재실행에서
    영원히 건너뛰어진다 — 이어받기가 필요한 바로 그 순간에 무너진다."""
    import requests as _rq
    from model import party as P
...
def test_load_refuses_a_broken_file_before_touching_the_db(tmpdir=None):
    """깨진 행을 건너뛰고 DELETE 가 돌면, 잘린 파일이 조용히 서빙 테이블을
    줄인다. DB 를 건드리기 전에 실패해야 한다."""
    import tempfile, pathlib, json as _j
    from model import party as P
...
if __name__ == "__main__":
    import sys as _s
    bad = 0
    bad += _t("fetch_failure_raises_instead_of_returning_empty",
              test_fetch_failure_raises_instead_of_returning_empty)
    bad += _t("load_refuses_a_broken_file_before_touching_the_db",
              test_load_refuses_a_broken_file_before_touching_the_db)
    print(f"\n수집 실패 게이트 {2 - bad}/2")
