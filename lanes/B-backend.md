...
`grid_score` **241,776행** (12업태 × 20,148격자). 레인 A가 생성. 조회는 인덱스 타므로 즉시.
...
`grid_score=241,776`, 곡선 표본 합계 `7,915`, 등급별 37점, 36개월 곡선과
...
2. **NULL은 NULL로.** 매출 없는 격자(49.5%)를 0으로 내보내면 UI가 "최악의 입지"로 그린다. `available: false` 같은 플래그로 구분.
